...
class VendorPaymentController extends Controller
{
    ...
    {
        $this->middleware(['vendor.auth', 'restrict.login']);
    ...
    public function fundAccount(Request $request)
    {
        ...
        try {
            ...
            $payload = [
                ...
                'ResponseUrl' => url('/api/v1/vendor/payments/fund/callback'), // Using API callback
                'FeeBearer' => 'Merchant', // Options: Customer, Merchant
                'MetaData' => $metadata,
            ];

            Log::debug('Initiating NABRoll transaction for vendor funding via API', ['payload' => $payload]);
            ...
            if ($response->failed() || !isset($result['status']) || $result['status'] !== 'SUCCESSFUL') {
                Log::error('NABRoll transaction initiation failed for vendor funding via API', [
                    'vendor_id' => $vendor->id,
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'Failed to initiate funding: ' . ($result['msg'] ?? 'Unknown error')
                ], 400);
            }
            ...
            Log::info('NABRoll transaction initiated for vendor funding via API', [
                'vendor_payment_id' => $vendorPayment->id,
                'transaction_ref' => $result['TransactionRef'],
                'payment_code' => $result['PaymentCode'],
            ...
            DB::rollBack();
            Log::error('Vendor funding initiation failed via API', ['error' => $e->getMessage()]);
            
            return response()->json([
                'success' => false,
                'message' => 'Funding initiation failed: ' . $e->getMessage()
            ], 500);
        }
    }

    /**
     * Process funding callback from API
     */
    public function fundCallback(Request $request)
    {
        $transactionRef = $request->query('TransactionRef');
        if (!$transactionRef) {
            Log::error('Missing transaction reference in vendor funding callback via API', ['query' => $request->query()]);
        ...
        if (!$vendorPayment) {
            Log::error('Vendor funding payment not found for transaction reference via API', ['transaction_ref' => $transactionRef]);
            
            return response()->json([
                'success' => false,
                'message' => 'Funding payment not found.'
            ], 404);
        }
        ...
        Log::debug('Verifying NABRoll transaction for vendor funding via API', ['payload' => $verifyPayload]);

        $response = Http::asForm()->post("{$this->baseUrl}/transactions/verify", $verifyPayload);
        $result = $response->json();

        Log::debug('NABRoll verify response for vendor funding via API', [
            'vendor_payment_id' => $vendorPayment->id,
        ...
        if ($response->failed() || !isset($result['status']) || $result['status'] !== 'SUCCESSFUL') {
            $vendorPayment->update(['payment_status' => 'FAILED', 'status' => 'failed']);
            Log::error('Vendor funding verification failed via API', [
                'vendor_payment_id' => $vendorPayment->id,
                'transaction_ref' => $transactionRef,
                'response' => $result,
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Funding verification failed: ' . ($result['msg'] ?? 'Unknown error')
            ], 400);
        }
        ...
        try {
            ...
            Log::info('Vendor account funded successfully via API', [
                'vendor_id' => $vendor->id,
                'vendor_payment_id' => $vendorPayment->id,
            ...
            DB::commit();
            Log::info('Vendor funding processed successfully via API', [
                'vendor_payment_id' => $vendorPayment->id,
                'transaction_ref' => $transactionRef,
            ...
            DB::rollBack();
            $vendorPayment->update(['payment_status' => 'FAILED', 'status' => 'failed']);
            Log::error('Vendor funding processing failed via API', ['error' => $e->getMessage()]);
            
            return response()->json([
                'success' => false,
                'message' => 'Funding processing failed: ' . $e->getMessage()
            ], 500);
        }
    }

    /**
     * Initiate payment for customer via API
     */
    ...
    /**
     * Process account payment internally
     */
    private function processAccountPayment($vendor, $customer, $amount)
    {
        ...
        Log::info('Vendor account payment processed successfully via API', [
            'vendor_payment_id' => $vendorPayment->id,
    ...
    /**
     * Initiate online payment internally
     */
    private function initiateOnlinePayment($vendor, $customer, $amount)
    {
        ...
        try {
            ...
            $payload = [
                ...
                'ResponseUrl' => url('/api/v1/vendor/payments/callback'), // Using API callback
                'FeeBearer' => 'Merchant', // Options: Customer, Merchant
                'MetaData' => $metadata,
            ];
            
            Log::debug('Initiating NABRoll transaction for vendor payment via API', ['payload' => $payload]);

            $response = Http::asForm()->timeout(30)->retry(2, 500)->post("{$this->baseUrl}/transactions/initiate", $payload);

            // Log the raw response for debugging
            Log::debug('NABRoll response details for vendor payment via API', [
            ...
            if ($response->failed() || is_null($result) || !isset($result['status']) || $result['status'] !== 'SUCCESSFUL') {
                Log::error('NABRoll transaction initiation failed for vendor payment via API', [
                    'vendor_id' => $vendor->id,
                    'vendor_payment_id' => $vendorPayment->id,
                    'status_code' => $response->status(),
                ...
                $errorMessage = $this->getSpecificErrorMessage((string)$response->body(), $response->status());
                return response()->json([
                    'success' => false,
            ...
            Log::info('NABRoll transaction initiated for vendor payment via API', [
                'vendor_payment_id' => $vendorPayment->id,
                'transaction_ref' => $result['TransactionRef'],
                'payment_code' => $result['PaymentCode'],
                'payment_url' => $result['PaymentUrl'],
            ...
            DB::rollBack();
            Log::error('Vendor payment initiation failed via API', ['error' => $e->getMessage()]);
    ...
    /**
     * Process payment callback from API
     */
    public function callback(Request $request)
    {
        $transactionRef = $request->query('TransactionRef');
        if (!$transactionRef) {
            Log::error('Missing transaction reference in vendor callback via API', ['query' => $request->query()]);
        ...
        if (!$vendorPayment) {
            Log::error('Vendor payment not found for transaction reference via API', ['transaction_ref' => $transactionRef]);
        ...
        Log::debug('Verifying NABRoll transaction for vendor payment via API', ['payload' => $verifyPayload]);

        $response = Http::asForm()->post("{$this->baseUrl}/transactions/verify", $verifyPayload);
        $result = $response->json();

        Log::debug('NABRoll verify response for vendor payment via API', [
            'vendor_payment_id' => $vendorPayment->id,
            'transaction_ref' => $transactionRef,
            'response' => $result,
        ]);
        ...
        if ($response->failed() || !isset($result['status']) || $result['status'] !== 'SUCCESSFUL') {
            $vendorPayment->update(['payment_status' => 'FAILED', 'status' => 'failed']);
            Log::error('Vendor payment verification failed via API', [
                'vendor_payment_id' => $vendorPayment->id,
                'transaction_ref' => $transactionRef,
                'response' => $result,
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Payment verification failed: ' . ($result['msg'] ?? 'Unknown error')
            ], 400);
        }
        ...
        try {
            ...
            Log::info('Vendor payment added to customer account balance via API', [
                'customer_id' => $customer->id,
                'vendor_payment_id' => $vendorPayment->id,
                'amount' => $vendorPayment->amount,
            ...
            DB::commit();
            Log::info('Vendor payment processed successfully via API', [
                'vendor_payment_id' => $vendorPayment->id,
                'customer_payment_id' => $customerPayment->id,
                'transaction_ref' => $transactionRef,
                'customer_new_account_balance' => $customer->account_balance,
            ...
            DB::rollBack();
            $vendorPayment->update(['payment_status' => 'FAILED', 'status' => 'failed']);
            Log::error('Vendor payment processing failed via API', ['error' => $e->getMessage()]);
    ...
    public function index(Request $request)
    {
        ...
        if ($perPage == 'all') {
            ...
            $vendorPayments = $query->orderBy('created_at', 'desc')->paginate($perPage);
        }
    ...
    /**
     * Get specific vendor payment
     */
    public function show(Request $request, $id)
    {
        $vendor = Auth::guard('vendor')->user();

        $payment = $vendor->vendorPayments()->with('customer')->find($id);
    ...
    /**
     * Get specific vendor funding record
     */
    public function showFunding(Request $request, $id)
    {
        $vendor = Auth::guard('vendor')->user();

        $funding = $vendor->vendorPayments()->funding()->find($id);
    ...
    /**
     * Get specific vendor payment record details
     */
    public function showPayment(Request $request, $id)
    {
        $vendor = Auth::guard('vendor')->user();

        $payment = $vendor->vendorPayments()->payments()->with('customer')->find($id);
    ...
    /**
     * Get specific vendor payment record details for a customer
     */
    public function customerPaymentDetails(Request $request, $customerId)
    {
        $vendor = Auth::guard('vendor')->user();

        $customer = $vendor->customers()->find($customerId);
        ...
        $payments = $vendor->vendorPayments()
    ...
    /**
     * Get specific vendor funding and payment statistics
     */
    public function statistics(Request $request)
    {
        ...
        $stats = [
            'total_payments' => $vendor->vendorPayments()->payments()->count(),
            'total_funding' => $vendor->vendorPayments()->funding()->count(),
            'successful_payments' => $vendor->vendorPayments()->payments()->where('payment_status', 'SUCCESSFUL')->count(),
            'successful_funding' => $vendor->vendorPayments()->funding()->where('payment_status', 'SUCCESSFUL')->count(),
            'total_paid_amount' => $vendor->vendorPayments()->payments()->sum('amount'),
            'total_funded_amount' => $vendor->vendorPayments()->funding()->sum('amount'),
            'recent_payments' => $vendor->vendorPayments()->payments()->orderBy('created_at', 'desc')->limit(5)->get(),
        ];
