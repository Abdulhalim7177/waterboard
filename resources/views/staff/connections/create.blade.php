            ...
            <div class="card-body pt-0">
                <form action="{{ route('staff.connections.store') }}" method="POST">
                    ...
                    <div class="fv-row mb-7">
                        ...
                        <textarea name="notes" id="notes" rows="3" class="form-control form-control-solid @error('notes') is-invalid @enderror">{{ old('notes') }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateConnectionSizeVisibility() {
        ...
        if(selectedTypeText.includes('legalisation') || selectedTypeText.includes('reconnection')) {
            ...
            // Show only relevant sizes based on connection type
